...
document.addEventListener('DOMContentLoaded', () => {
    const elementsToAnimate = document.querySelectorAll(
        '.skill-category, .project-card, .contact-item, .stat, .about-text'
    );
...
function typeWriterHTML(element, speed = 200) {
    const text1 = "Backend & DevOps";
    const text2 = "Engineer";
    ...
    function typeFirstLine() {
        if (i < text1.length) {
            ...
            element.innerHTML = `<span class="gradient-text">${currentText}</span>`;
            i++;
            setTimeout(typeFirstLine, speed);
        } else {
            // Add line break and start second line
            element.innerHTML = `<span class="gradient-text">${text1}</span><br>`;
    ...
    function typeSecondLine() {
        if (j < text2.length) {
            ...
            element.innerHTML = `<span class="gradient-text">${text1}</span><br>${currentText}`;
            j++;
